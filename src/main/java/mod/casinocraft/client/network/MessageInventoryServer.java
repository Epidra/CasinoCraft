...
public class MessageInventoryServer {
	...
	public static class Handler {
		public static void handle (final MessageInventoryServer message, Supplier<NetworkEvent.Context> context) {
			...
			PacketHandler.sendToChunk(new MessageInventoryClient(inv, message.storageToken, message.storagePrize, message.pos), context.get().getSender().level().getChunkAt(message.pos));
