		...
		public static void handle (final MessagePlayerServer message, Supplier<NetworkEvent.Context> context) {
			...
			context.get().enqueueWork(() ->{
				if(message.amount < 0){
					...
					serverPlayer.getInventory().add(new ItemStack(message.stack.getItem(), message.amount));
				}
			});
			
			// PacketHandler.sendTo(message, serverPlayer);
			// PacketHandler.sendToChunk(new MessageScoreClient(message.names, message.points, message.pos), context.get().getSender().level().getChunkAt(message.pos));
