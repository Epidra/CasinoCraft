    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0){
        	if(mouseRect(80, 3, 16, 16, mouseX, mouseY)) { // Low Bet PLUS
        		tileArcade.bet_low++;
        		tileArcade.bet_high++;
        		CasinoPacketHandler.INSTANCE.sendToServer(new ServerBoardMessage(tileArcade.bet_low, tileArcade.bet_high, tileArcade.transfer_in, tileArcade.transfer_out, false, tileArcade.getPos()));
        	}
        	...
        	if(mouseRect(43, 15, 18, 18, mouseX, mouseY)) { // Transfer IN
        		...
        		if(!tileArcade.transfer_in) {
        			CasinoPacketHandler.INSTANCE.sendToServer(new ServerBlockMessage(tileArcade.inventory.get(0), tileArcade.inventory.get(1), tileArcade.inventory.get(4), tileArcade.bet_storage, tileArcade.getPos()));
        		}
        	}
