...
public class GuiBlackJack extends GuiCasino {
    ...
    protected void mouseClicked2(double mouseX, double mouseY, int mouseButton){
        ...
        if(playerToken >= bet || !CONTAINER.hasToken())
            if(logic().turnstate == 2 && mouseRect( 24, 204-40, 92, 26, mouseX, mouseY)){ CollectBet(); action(2); }
    }
    ...
    protected void drawGuiContainerForegroundLayer2(int mouseX, int mouseY){
        ...
        if(logic().turnstate >= 3) drawString("DEALER:  " + logic().cards_dealer, 24, 56);
    }
    ...
    protected void drawGuiContainerBackgroundLayer2(float partialTicks, int mouseX, int mouseY){
        ...
        if(logic().turnstate == 2){
            ...
            if(logic().cards_player1.get(0).number == logic().cards_player1.get(1).number){
