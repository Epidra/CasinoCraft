...
public class GuiColumns extends GuiCasino {
    ...
    protected void drawGuiContainerBackgroundLayer2(float partialTicks, int mouseX, int mouseY){
        ...
        if(logic().turnstate >= 2){
            ...
            this.blit(guiLeft+8 + 16*logic().tromino[0].X+16+16, guiTop+8 + 16*logic().tromino[0].Y, logic().turnstate >= 4 ? 16*8 : logic().turnstate >= 4 ?  8 : 16*logic().container_current[0], 216-24, 16, 16);
            this.blit(guiLeft+8 + 16*logic().tromino[1].X+16+16, guiTop+8 + 16*logic().tromino[1].Y, logic().turnstate >= 4 ? 16*8 : logic().turnstate >= 4 ?  8 : 16*logic().container_current[1], 216-24, 16, 16);
            this.blit(guiLeft+8 + 16*logic().tromino[2].X+16+16, guiTop+8 + 16*logic().tromino[2].Y, logic().turnstate >= 4 ? 16*8 : logic().turnstate >= 4 ?  8 : 16*logic().container_current[2], 216-24, 16, 16);
    ...
    private void drawTetromino(int mino0, int mino1, int mino2, int x, int y) {
        this.blit(guiLeft+x + 48/2, guiTop+y +  0  , logic().turnstate >= 4 ? 16*8 : 16*mino0, 216-24, 16, 16);
        this.blit(guiLeft+x + 48/2, guiTop+y + 32/2, logic().turnstate >= 4 ? 16*8 : 16*mino1, 216-24, 16, 16);
        this.blit(guiLeft+x + 48/2, guiTop+y + 64/2, logic().turnstate >= 4 ? 16*8 : 16*mino2, 216-24, 16, 16);
    }
