...
public class GuiMeanMinos extends GuiCasino {
    ...
    protected void drawGuiContainerBackgroundLayer2(float partialTicks, int mouseX, int mouseY){
        ...
        if(logic().turnstate >= 2){
            ...
            this.blit(guiLeft+8+16+16 + 16*logic().domino[0].X, guiTop+8 + 16*logic().domino[0].Y, logic().turnstate >= 4 ? 16*8 : 16*logic().turnstate >= 4 ?  8 : logic().container_current[0], 216, 16, 16);
            this.blit(guiLeft+8+16+16 + 16*logic().domino[1].X, guiTop+8 + 16*logic().domino[1].Y, logic().turnstate >= 4 ? 16*8 : 16*logic().turnstate >= 4 ?  8 : logic().container_current[1], 216, 16, 16);
    ...
    private void drawTetromino(int mino0, int mino1, int x, int y) {
        ...
        this.blit(guiLeft+x + 48/2, guiTop+y + 64/2, logic().turnstate >= 4 ? 16*8 : 16*mino0, 216, 16, 16);
    }
