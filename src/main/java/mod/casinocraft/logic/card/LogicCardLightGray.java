...
public class LogicCardLightGray extends LogicBase {   // Draw Poker
    ...
    public void updateLogic() {
        timeout++;
        if(turnstate == 2 && timeout >= CasinoKeeper.config_timeout || getFirstFreePlayerSlot() == (tableID == 1 ? 4 : 6)){
            draw();
        }
        if(turnstate == 3){
    ...
    public void load2(NBTTagCompound compound){
        ...
        pot = compound.getInteger("pot");
        raisedPlayer = compound.getInteger("raisedplayer");
    }
    ...
    public NBTTagCompound save2(NBTTagCompound compound){
        ...
        compound.setBoolean("folded0", folded[0]);
        compound.setBoolean("folded1", folded[1]);
        compound.setBoolean("folded2", folded[2]);
        compound.setBoolean("folded3", folded[3]);
        compound.setBoolean("folded4", folded[4]);
        compound.setBoolean("folded0", folded[0]);
        compound.setBoolean("dropped1", dropped[1]);
        compound.setBoolean("dropped2", dropped[2]);
        compound.setBoolean("dropped3", dropped[3]);
        compound.setBoolean("dropped4", dropped[4]);
        compound.setInteger("pot", pot);
        compound.setInteger("raisedplayer", raisedPlayer);
        return compound;
    }
