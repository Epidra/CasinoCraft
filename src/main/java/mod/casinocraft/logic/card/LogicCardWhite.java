...
public class LogicCardWhite extends LogicBase {   // Single Poker
    ...
    public void load2(NBTTagCompound compound){
        ...
        movestate = compound.getInteger("movestate");
    }

    public NBTTagCompound save2(NBTTagCompound compound){
        saveCardArray(compound, 0, cards_field);
        compound.setBoolean("hold0", hold[0]);
        compound.setBoolean("hold1", hold[1]);
        compound.setBoolean("hold2", hold[2]);
        compound.setBoolean("hold3", hold[3]);
        compound.setBoolean("hold4", hold[4]);
        compound.setInteger("ticker", ticker);
        compound.setInteger("movestate", movestate);
        return compound;
    }
