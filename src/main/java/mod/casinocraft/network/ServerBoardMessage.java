...
public class ServerBoardMessage implements IMessage {
    ...
    public static class Handler implements IMessageHandler<ServerBoardMessage, IMessage> {
    	
    	@Override
        public IMessage onMessage (final ServerBoardMessage message, final MessageContext context) {
    		...
    		CasinoPacketHandler.INSTANCE.sendToAll(new ClientBoardMessage(message.betLow, message.betHigh, message.transferIn, message.transferOut, message.isCreative, pos));
    		// No response packet
            return null;
        }
    }
