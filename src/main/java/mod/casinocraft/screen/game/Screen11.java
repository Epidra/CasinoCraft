...
import mod.casinocraft.logic.game.Logic11;
	...
	protected void drawBackgroundLayer(PoseStack matrix, float partialTicks, int mouseX, int mouseY){
		...
		for(int y = 0; y < 7; y++){
			for(int x = 0; x < 25; x++){
				...
				int posY = y == 6 ? 200 : y == 5 ? 168 : 32 + 24*y;
				if(color == -1                   ){ blit(matrix, leftPos+posX, topPos+posY, 224,               224, 32, 32); }
				if(color  >  0                   ){ blit(matrix, leftPos+posX, topPos+posY, 192, 32 * (color % 10), 32, 32); }
				if(logic().selector.matches(x, y)){ blit(matrix, leftPos+posX, topPos+posY, 224,                 0, 32, 32); }
			}
		}
