...
public class TileEntityRoulette extends TileEntityCasino {
    ...
    public void actionTouch(int action){
    	...
    	if(action == -1){
    		...
    		selector.set(action%25, action/25);
    	}
    }
	...
	public int getValue(int index){
		...
		if(index == -2) return reward;
		return gridI[index%25][index/25];
	}
    ...
    private void Place() { // Unused
        ...
        //    if(GridValid(selector) && gridI[(int)selector.X][(int)selector.Y] == 0) selector = selector;
        //} else {
            gridI[selector.X][selector.Y] = 1;
            selector = new Vector2(-1, -1);
            //if(FM.coins >= bet) {
    ...
    private void Spin() {
        if(turnstate == 2 && !spinning) {
            if(selector.X != -1) gridI[selector.X][selector.Y] = 1;
            turnstate = 3;
        } else if(turnstate == 3 && !spinning) {
            timer = 100 + rand.nextInt(500);
            spinning = true;
