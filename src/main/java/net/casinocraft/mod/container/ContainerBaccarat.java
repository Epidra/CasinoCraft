...
public class ContainerBaccarat extends Container{
    ...
    public ContainerBaccarat(InventoryPlayer playerInventory, IInventory blockInventory){
    ...
    public void detectAndSendChanges(){
        ...
        for (int i = 0; i < this.listeners.size(); ++i){
            ...
            if(this.status        != this.tileCardTable.status)       { icontainerlistener.sendProgressBarUpdate(this, 2, 0); }
            if(this.end           != this.tileCardTable.end)          { icontainerlistener.sendProgressBarUpdate(this, 3, 0); }
            if(this.active_player != this.tileCardTable.active_player){ icontainerlistener.sendProgressBarUpdate(this, 4, 0); }
            ...
            if(this.cards_dealer.size() != this.tileCardTable.cards_dealer.size()){
            	icontainerlistener.sendProgressBarUpdate(this, 6, 0);
            } else {
        ...
        this.status        = this.tileCardTable.status;
    ...
    public void updateProgressBar(int id, int data){
    	...
    	if(id == 2)  this.tileCardTable.status        = this.status;
    	if(id == 3)  this.tileCardTable.end           = this.end;
    	if(id == 4)  this.tileCardTable.active_player = this.active_player;
    	if(id == 5){ this.tileCardTable.cards_player.clear(); 
    	this.tileCardTable.cards_player.addAll(this.cards_player); } 
    	if(id == 6){ this.tileCardTable.cards_dealer.clear(); this.tileCardTable.cards_dealer.addAll(this.cards_dealer); } 
    }
