...
public class GuiBlackJack extends GuiScreen
{
    ...
    private ResourceLocation textureGround = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/BlackJack.png");
	...
	TileEntityBlackJack entity;
	boolean pressed = false;
    
    public GuiBlackJack(InventoryPlayer inventoryPlayer, TileEntityBlackJack tileentity){
    	//super(new ContainerVideoPoker(inventoryPlayer, tileentity));
    	
		entity = tileentity;
		this.xSize = 256;
		this.ySize = 256;
		this.inventorySlots = new ContainerBlackJack(inventoryPlayer, tileentity);
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_){
		...
		if(entity.active_bet && !entity.active_won){
			drawTexturedModalRect(guiLeft+82, guiTop+161, 0, 26, 92, 26); // Button Start
		...
		if(!entity.active_bet && entity.active_won){
			...
			if(entity.field == 1){
				...
				drawTexturedModalRect(guiLeft+250 - 16*1, guiTop+197, 5, 0, 16, 26); // Slot 1 Middle
				drawTexturedModalRect(guiLeft+250 - 16*2, guiTop+197, 5, 0, 16, 26); // Slot 1 Middle
				drawTexturedModalRect(guiLeft+250 - 16*2 - 5, guiTop+197, 0, 0, 5, 26); // Slot 1 Begin
			}
		}
		...
		if(entity.active_bet){
			...
			for(int i = 0; i < 10; i++){
				if(entity.cardPx[i] != -1){
					if(entity.cardPy[i] == 0){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureHerz);  drawTexturedModalRect(guiLeft+8+10*i, guiTop+78+5*i, POS(true,entity.cardPx[i]), POS(false,entity.cardPx[i]), 42, 57); }
					if(entity.cardPy[i] == 1){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureKaro);  drawTexturedModalRect(guiLeft+8+10*i, guiTop+78+5*i, POS(true,entity.cardPx[i]), POS(false,entity.cardPx[i]), 42, 57); }
					if(entity.cardPy[i] == 2){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureKreuz); drawTexturedModalRect(guiLeft+8+10*i, guiTop+78+5*i, POS(true,entity.cardPx[i]), POS(false,entity.cardPx[i]), 42, 57); }
					if(entity.cardPy[i] == 3){ Minecraft.getMinecraft().getTextureManager().bindTexture(texturePik);   drawTexturedModalRect(guiLeft+8+10*i, guiTop+78+5*i, POS(true,entity.cardPx[i]), POS(false,entity.cardPx[i]), 42, 57); }
				}
				if(entity.cardDx[i] != -1){
					if(entity.cardDy[i] == 0){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureHerz);  drawTexturedModalRect(guiLeft+140+10*i, guiTop+17+5*i, POS(true,entity.cardDx[i]), POS(false,entity.cardDx[i]), 42, 57); }
					if(entity.cardDy[i] == 1){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureKaro);  drawTexturedModalRect(guiLeft+140+10*i, guiTop+17+5*i, POS(true,entity.cardDx[i]), POS(false,entity.cardDx[i]), 42, 57); }
					if(entity.cardDy[i] == 2){ Minecraft.getMinecraft().getTextureManager().bindTexture(textureKreuz); drawTexturedModalRect(guiLeft+140+10*i, guiTop+17+5*i, POS(true,entity.cardDx[i]), POS(false,entity.cardDx[i]), 42, 57); }
					if(entity.cardDy[i] == 3){ Minecraft.getMinecraft().getTextureManager().bindTexture(texturePik);   drawTexturedModalRect(guiLeft+140+10*i, guiTop+17+5*i, POS(true,entity.cardDx[i]), POS(false,entity.cardDx[i]), 42, 57); }
				}
			}
			if(!entity.active_bet && !entity.active_won){
				Minecraft.getMinecraft().getTextureManager().bindTexture(textureHerz);  drawTexturedModalRect(guiLeft+140+10*1, guiTop+17+5*1, 0, 0, 42, 57);
			}
		}
		if(entity.active_won){
			Minecraft.getMinecraft().getTextureManager().bindTexture(textureExtra);
			drawTexturedModalRect(guiLeft, guiTop+88, 0, 60 + 19*entity.hand, 256, 20); // Text 0
		}
    }
    ...
    protected void mouseMovedOrUp(int mouseX, int mouseY, int moveorup)
    {
        ...
        if(moveorup != -1){
        	if(!entity.active_bet && !entity.active_won && guiLeft+4 < mouseX && mouseX < guiLeft+4+92 && guiTop+166 < mouseY && mouseY < guiTop+166+26){
        		entity.Click_Hit();
    		}else
			if( entity.active_bet && !entity.active_won && guiLeft+82 < mouseX && mouseX < guiLeft+82+92 && guiTop+161 < mouseY && mouseY < guiTop+161+26){
				entity.Click_Start(0);
    		}else
			if(!entity.active_bet &&  entity.active_won && guiLeft+82 < mouseX && mouseX < guiLeft+82+92 && guiTop+161 < mouseY && mouseY < guiTop+161+26){
				entity.Click_Start(1);
    		}else
			if(!entity.active_bet && !entity.active_won && guiLeft+160 < mouseX && mouseX < guiLeft+160+92 && guiTop+166 < mouseY && mouseY < guiTop+166+26){
				entity.Click_Stand();
    		}
        }
