...
public class GuiColumns extends GuiContainer{
	
    /** The player inventory bound to this GUI. */
    private final InventoryPlayer playerInventory;
    private final TileEntityColumns tileFurnace;
    ...
    public GuiColumns(InventoryPlayer playerInv, IInventory furnaceInv){
        super(new ContainerColumns(playerInv, furnaceInv));
        this.playerInventory = playerInv;
        this.tileFurnace = (TileEntityColumns) furnaceInv;
        this.xSize = 256;
		this.ySize = 256;
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException{
    	...
    	if(keyCode == Keyboard.KEY_DOWN) { tileFurnace.Column_Fall(); }
    	if(keyCode == Keyboard.KEY_M)    { tileFurnace.Command_Cycle("left"); }
    	if(keyCode == Keyboard.KEY_N)    { tileFurnace.Command_Cycle("right"); }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY){
        ...
        if(tileFurnace.grid_color[0][0] != null){
        	...
        	this.drawTexturedModalRect(guiLeft+8+45 + 16*tileFurnace.tetromino[2].X, guiTop+3 + 16*tileFurnace.tetromino[2].Y, MinoPos(tileFurnace.container_current[2], true), MinoPos(tileFurnace.container_current[2], false), 16, 16);
        }
