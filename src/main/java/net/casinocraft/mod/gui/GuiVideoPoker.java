...
public class GuiVideoPoker extends GuiScreen
{
    ...
    private ResourceLocation textureGround = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/VideoPoker.png");
	private ResourceLocation textureExtra  = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/VideoPokerExtra.png");
	private ResourceLocation textureHerz   = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/Cards_Herz.png");
	private ResourceLocation textureKaro   = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/Cards_Karo.png");
	private ResourceLocation textureKreuz  = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/Cards_Kreuz.png");
	private ResourceLocation texturePik    = new ResourceLocation(CasinoCraft.modid + ":" + "textures/gui/Cards_Pik.png");
	
	TileEntityVideoPoker entity;
	boolean pressed = false;
    
    public GuiVideoPoker(InventoryPlayer inventoryPlayer, TileEntityVideoPoker tileentity){
    	//super(new ContainerVideoPoker(inventoryPlayer, tileentity));
    	
		entity = tileentity;
		this.xSize = 256;
		this.ySize = 256;
		this.inventorySlots = new ContainerVideoPoker(inventoryPlayer, tileentity);
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_){
		...
		Minecraft.getMinecraft().getTextureManager().bindTexture(textureGround);
		drawTexturedModalRect(guiLeft, guiTop, 0, 0, xSize, ySize); // Background
		Minecraft.getMinecraft().getTextureManager().bindTexture(textureExtra);
		if(entity.active_bet && !entity.active_won){
			drawTexturedModalRect(guiLeft+82, guiTop+164, 0, 26, 92, 26); // Button Start
			drawTexturedModalRect(guiLeft+4, guiTop+197, 0, 0, 26, 26); // Slot 0
		}
		if(!entity.active_bet && !entity.active_won){
			drawTexturedModalRect(guiLeft+82, guiTop+164, 118, 0, 92, 26); // Button Finish
			if(!entity.hold1){drawTexturedModalRect(guiLeft+5, guiTop+16, 26, 0, 46, 26);}else{drawTexturedModalRect(guiLeft+5, guiTop+16, 72, 0, 46, 26);} // Button Hold 1
			if(!entity.hold2){drawTexturedModalRect(guiLeft+55, guiTop+16, 26, 0, 46, 26);}else{drawTexturedModalRect(guiLeft+55, guiTop+16, 72, 0, 46, 26);} // Button Hold 2
			if(!entity.hold3){drawTexturedModalRect(guiLeft+105, guiTop+16, 26, 0, 46, 26);}else{drawTexturedModalRect(guiLeft+105, guiTop+16, 72, 0, 46, 26);} // Button Hold 3
			if(!entity.hold4){drawTexturedModalRect(guiLeft+155, guiTop+16, 26, 0, 46, 26);}else{drawTexturedModalRect(guiLeft+155, guiTop+16, 72, 0, 46, 26);} // Button Hold 4
			if(!entity.hold5){drawTexturedModalRect(guiLeft+205, guiTop+16, 26, 0, 46, 26);}else{drawTexturedModalRect(guiLeft+205, guiTop+16, 72, 0, 46, 26);} // Button Hold 5
		}
		if(!entity.active_bet && entity.active_won){
			drawTexturedModalRect(guiLeft+82, guiTop+164, 92, 26, 92, 26); // Button New Game
			drawTexturedModalRect(guiLeft, guiTop+28, 0, 60 + 19*entity.hand, 256, 20); // Text 0
			//drawTexturedModalRect(guiLeft+82, guiTop+164, 118, 0, 92, 26); // Button Finish
			if(entity.field != 0){
				drawTexturedModalRect(guiLeft+250, guiTop+197, 21, 0, 5, 26); // Slot 1 End
				for(int i = 1; i <= entity.field; i++){
					drawTexturedModalRect(guiLeft+250 - 16*i, guiTop+197, 5, 0, 16, 26); // Slot 1 Middle
				}
				drawTexturedModalRect(guiLeft+250 - 16*entity.field - 5, guiTop+197, 0, 0, 5, 26); // Slot 1 Begin
			}
		}
		
		if(entity.active_bet){
			Minecraft.getMinecraft().getTextureManager().bindTexture(textureHerz);
			drawTexturedModalRect(guiLeft+7, guiTop+72, 0, 0, 42, 57); // Card Cover
			drawTexturedModalRect(guiLeft+57, guiTop+72, 0, 0, 42, 57); // Card Cover
			drawTexturedModalRect(guiLeft+107, guiTop+72, 0, 0, 42, 57); // Card Cover
			drawTexturedModalRect(guiLeft+157, guiTop+72, 0, 0, 42, 57); // Card Cover
			drawTexturedModalRect(guiLeft+207, guiTop+72, 0, 0, 42, 57); // Card Cover
		}else{
			Minecraft.getMinecraft().getTextureManager().bindTexture(textureHerz);
			if(entity.card1y == 0) drawTexturedModalRect(guiLeft+7, guiTop+72, POS(true,entity.card1x), POS(false,entity.card1x), 42, 57); // Card 1 Herz
			if(entity.card2y == 0) drawTexturedModalRect(guiLeft+57, guiTop+72, POS(true,entity.card2x), POS(false,entity.card2x), 42, 57); // Card 2 Herz
			if(entity.card3y == 0) drawTexturedModalRect(guiLeft+107, guiTop+72, POS(true,entity.card3x), POS(false,entity.card3x), 42, 57); // Card 3 Herz
			if(entity.card4y == 0) drawTexturedModalRect(guiLeft+157, guiTop+72, POS(true,entity.card4x), POS(false,entity.card4x), 42, 57); // Card 4 Herz
			if(entity.card5y == 0) drawTexturedModalRect(guiLeft+207, guiTop+72, POS(true,entity.card5x), POS(false,entity.card5x), 42, 57); // Card 5 Herz
			Minecraft.getMinecraft().getTextureManager().bindTexture(textureKaro);
			if(entity.card1y == 1) drawTexturedModalRect(guiLeft+7, guiTop+72, POS(true,entity.card1x), POS(false,entity.card1x), 42, 57); // Card 1 Karo
			if(entity.card2y == 1) drawTexturedModalRect(guiLeft+57, guiTop+72, POS(true,entity.card2x), POS(false,entity.card2x), 42, 57); // Card 2 Karo
			if(entity.card3y == 1) drawTexturedModalRect(guiLeft+107, guiTop+72, POS(true,entity.card3x), POS(false,entity.card3x), 42, 57); // Card 3 Karo
			if(entity.card4y == 1) drawTexturedModalRect(guiLeft+157, guiTop+72, POS(true,entity.card4x), POS(false,entity.card4x), 42, 57); // Card 4 Karo
			if(entity.card5y == 1) drawTexturedModalRect(guiLeft+207, guiTop+72, POS(true,entity.card5x), POS(false,entity.card5x), 42, 57); // Card 5 Karo
			Minecraft.getMinecraft().getTextureManager().bindTexture(textureKreuz);
			if(entity.card1y == 2) drawTexturedModalRect(guiLeft+7, guiTop+72, POS(true,entity.card1x), POS(false,entity.card1x), 42, 57); // Card 1 Kreuz
			if(entity.card2y == 2) drawTexturedModalRect(guiLeft+57, guiTop+72, POS(true,entity.card2x), POS(false,entity.card2x), 42, 57); // Card 2 Kreuz
			if(entity.card3y == 2) drawTexturedModalRect(guiLeft+107, guiTop+72, POS(true,entity.card3x), POS(false,entity.card3x), 42, 57); // Card 3 Kreuz
			if(entity.card4y == 2) drawTexturedModalRect(guiLeft+157, guiTop+72, POS(true,entity.card4x), POS(false,entity.card4x), 42, 57); // Card 4 Kreuz
			if(entity.card5y == 2) drawTexturedModalRect(guiLeft+207, guiTop+72, POS(true,entity.card5x), POS(false,entity.card5x), 42, 57); // Card 5 Kreuz
			Minecraft.getMinecraft().getTextureManager().bindTexture(texturePik);
			if(entity.card1y == 3) drawTexturedModalRect(guiLeft+7, guiTop+72, POS(true,entity.card1x), POS(false,entity.card1x), 42, 57); // Card 1 Pik
			if(entity.card2y == 3) drawTexturedModalRect(guiLeft+57, guiTop+72, POS(true,entity.card2x), POS(false,entity.card2x), 42, 57); // Card 2 Pik
			if(entity.card3y == 3) drawTexturedModalRect(guiLeft+107, guiTop+72, POS(true,entity.card3x), POS(false,entity.card3x), 42, 57); // Card 3 Pik
			if(entity.card4y == 3) drawTexturedModalRect(guiLeft+157, guiTop+72, POS(true,entity.card4x), POS(false,entity.card4x), 42, 57); // Card 4 Pik
			if(entity.card5y == 3) drawTexturedModalRect(guiLeft+207, guiTop+72, POS(true,entity.card5x), POS(false,entity.card5x), 42, 57); // Card 5 Pik
		}
    }
     ...
     */
    protected void mouseMovedOrUp(int mouseX, int mouseY, int moveorup)
    {
        super.mouseMovedOrUp(mouseX, mouseY, moveorup); //Forge, Call parent to release buttons
        ...
        if (this.field_146993_M && slot != null && moveorup == 0 && this.inventorySlots.func_94530_a((ItemStack)null, slot))
        {
            if (isShiftKeyDown())
            {
                if (slot != null && slot.inventory != null && this.field_146994_N != null)
                {
                    ...
                    while (iterator.hasNext())
                    {
                        ...
                        {
                            this.handleMouseClick(slot1, slot1.slotNumber, moveorup, 1);
                        }
                    }
                }
            }
            ...
            {
                this.handleMouseClick(slot, j1, moveorup, 6);
            }
        ...
        {
            if (this.field_147007_t && this.field_146988_G != moveorup)
            {
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
            {
                if (moveorup == 0 || moveorup == 1)
                {
                    ...
                    {
                        this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, moveorup, 0);
            ...
            {
                if (moveorup == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100)
                {
                    this.handleMouseClick(slot, j1, moveorup, 3);
                }
                else
                {
                    ...
                    this.handleMouseClick(slot, j1, moveorup, flag1 ? 1 : 0);
                }
            }
        }
