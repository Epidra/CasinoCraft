...
public class TileEntityCasino extends TileEntity implements ITickable, IInventory{
    ...
    public ItemStack decrStackSize(int index, int count){
        return ItemStackHelper.getAndSplit(this.arcadeItemStacks, index, count);
    }
    
    /** Removes a stack from the given slot and returns it. */
    @Nullable
    public ItemStack removeStackFromSlot(int index){
        return ItemStackHelper.getAndRemove(this.arcadeItemStacks, index);
    }
    ...
    public void update(){
    	if(this.worldObj.getBlockState(getPos()).getBlock() == CasinoKeeper.blockCardTable){
    		color = this.worldObj.getBlockState(getPos()).getValue(BlockCardTable.COLOR);
    	}
    	if(content == null){
